...
public class RequestUtils {
	...
	public static String getQueryParam(HttpServletRequest request, String name) {
		...
		if (request.getMethod().equalsIgnoreCase(WebStatic.POST)) {
		...
		try {
			s = URLDecoder.decode(s, WebStatic.CHARSET);
	...
	public static String getRequestedSessionId(HttpServletRequest request) {
		...
		if (request.isRequestedSessionIdFromURL() || StringUtils.isBlank(ctx)) {
			...
			// 手动从cookie获取
			Cookie cookie = CookieUtils.getCookie(request,WebStatic.JSESSION_COOKIE);
			if (cookie != null) {
	...
	/**
	 * 把所有收到的请求都放到modelMap中
	 * @param request
	 * @param model
	 */
	public static void pubAllParamInModelMap(HttpServletRequest request, ModelMap model){
...
//		}
	 ...
	 * @return
	 */
	public static String injection(String sql){
