...
import com.zmax.mag.domain.dao.my.WxrbRepo;
 ...
 */
@Service
public class WxrbService extends BaseService<Wxrb> {
	/**日志实例*/
	...
	/**扩展版*/
	@Autowired
	BaseRepo<Wxrb,Integer> baseRepo;
	@Autowired
	 ...
	 * @return 返回相应的持久化PO实例
	 */
	public Wxrb get(User sessionUser,Integer id) {
		if(id==null)
			return null;
		if(id.intValue()==0)
			return null;
		Wxrb obj= wxrbRepo.findOne(id);
		if(obj==null)
			return null;
		if(!permitCheckUtils.checkObj(sessionUser, obj,"Wxrb")) 
			return null;
		return obj;
	}
	 ...
	 * @param params 其它参数
	 */
	public Wxrb getByKvIfNotExistsSaveKv(User sessionUser,Wxrb entity,String keyfield,Object value,String order, Object[] params) {
		if(entity==null)
			return null;
		if(!permitCheckUtils.checkObj(sessionUser, entity,"Wxrb")) return entity;
		Wxrb obj=getFirst(sessionUser, keyfield+"='"+value.toString()+"'", order, params);
		if(obj==null){
	 ...
	 * @return List
	 */
	public List<Wxrb> listfindByIn(User sessionUser,Integer[] ids,String morecondition) {
		String where=SqlUtils.whereFromIds(ids);
		where=SqlUtils.whereAdd(where,permitCheckUtils.addWhere(sessionUser,"Wxrb"));
		if(StringUtils.isNotBlank(morecondition))
			where=SqlUtils.whereAdd(where, morecondition);
		List<Integer> list=new ArrayList<Integer>();
		return wxrbRepo.findAll(list);
	}
	 ...
	 * @param wxrb
	 */
	public void save(User sessionUser,Wxrb wxrb){
		wxrbRepo.save(wxrb);
	}
	/**
	 * 更新
	 ...
	 * @param wxrb
	 */
	public void update(User sessionUser,Wxrb wxrb){
		wxrbRepo.save(wxrb);
	}
	/**
	 * 保存，新建或更新，有ID就更新，否则新建
	 ...
	 * @param wxrb
	 */
	public void saveOrUpdate(User sessionUser,Wxrb wxrb){
		save(sessionUser,wxrb);
	}
	/**
	 ...
	 * @param entities
	 */
	public void saveOrUpdateAll(User sessionUser,Collection<Wxrb> entities) {
		if(entities==null)
			return;
		for (Wxrb wxrb : entities) {
			save(sessionUser,wxrb);
		}
	}
	/**
	 ...
	 * @throws Exception
	 */
	public void deleteAll(User sessionUser,List<Wxrb> list) throws BoException,Exception{
		for (Wxrb obj : list) {
	 ...
	 */
	public void csindex(User sessionUser,Wxrb obj,String act) throws BoException,Exception{
		if(StringUtils.equals(act, "nullCreate")){
			if(obj==null)
				throw new Exception("无法创建空对象");
			Wxrb dbobj=get(sessionUser, obj.getId());
			if(dbobj==null){
				//创建_写表前操作之自定义，目的是修改obj
				commonzSvrUitlsSpec.createAndUpdateBefore("create",new Wxrb(), obj,sessionUser);
	 ...
	 * @return
	 */
	public Grid csgrid(User sessionUser,Wxrb search, PageHelper ph,String queryhq) throws BoException,Exception{
	 ...
	 * @throws Exception
	 */
	public void saveCreate(User sessionUser,Wxrb oldobj,Wxrb obj,String copyfrom,String copyfromwhere) throws BoException,Exception{
		//用于做复制新增的检查
		if(StringUtils.isNotBlank(copyfrom)&&StringUtils.isNotBlank(copyfromwhere)){
			...
			commonzSvrUitlsSpec.obj2objcheck(sessionUser,objsrc, obj ,copyfrom ,"Wxrb");
		}		

		//创建_写表前操作之通用
		commonzSvrUitlsSpec.commonCreateBefore(oldobj,obj,"Wxrb",sessionUser);
		//创建_写表前操作之本地
		...
		//TODO:lang
		//创建_写表后操作之本地
		createAndUpdateAfter(sessionUser,"create",obj,new Wxrb());
		//创建_写表后操作之自定义
		commonzSvrUitlsSpec.createAndUpdateAfter("create",obj,new Wxrb(),sessionUser);
	}
	/**
	 * 更新
	 ...
	 * @throws Exception
	 */
	public void updateUpdate(User sessionUser,Wxrb obj,Integer id) throws BoException,Exception{
		//数据库表里取出的对象，要把页面对象obj里的内容，放进来进行保存
		Wxrb dbobj=get(sessionUser, id);
		if(dbobj==null)
			throw new BoException("数据库中数据找不到");

		//更新_写表前操作之通用，目的是修改obj
		commonzSvrUitlsSpec.commonUpdateBefore(dbobj,obj,"Wxrb",sessionUser);
	 ...
	 * @throws Exception
	 */
	public void saveCreateUpdateUpdate(User sessionUser,Wxrb oldobj,Wxrb obj) throws BoException,Exception{
		if(null==obj.getId()){
	...
	public void updateBatchByIds(User sessionUser,String[] ids,String setfield) throws BoException,Exception{
		String hql= "update Wxrb set "+setfield +" where "+StringUtilz.whereIn(ids);
		execute(sessionUser, hql, null);
		commonzSvrUitlsSpec.batchupdateAfter(sessionUser,"Wxrb", setfield, ids);
	}
	/**
	 * 根据ID删除一个
	 * @param sessionUser 
	...
	public void deleteOneById(User sessionUser,Integer id) throws BoException,Exception{
		commonzSvrUitlsSpec.deleteByIdBefore(sessionUser,"Wxrb", id);
		deleteById(sessionUser,id);
		commonzSvrUitlsSpec.deleteByIdAfter(sessionUser,"Wxrb", id);
		//TODO:lang
	}
	/**
	 * 根据IDs删除批量
	 ...
	 * @return
	 */
	public Wxrb csshow(User sessionUser,Integer id) throws BoException,Exception{
		Wxrb wxrb=get(sessionUser, id);
		showAfter(sessionUser,wxrb);
		return (wxrb==null)?new Wxrb():wxrb;
	}
	/**
	 * 新建GET
	 * @param sessionUser 
	 ...
	 * @return
	 */
	public Wxrb csnewget(User sessionUser,Wxrb wxrb,String copyfrom,String copyfromwhere) throws BoException,Exception{
		if(wxrb==null)
			wxrb=new Wxrb();
		//如果是从其它表复制过来的
		if(StringUtils.isNotBlank(copyfrom)&&StringUtils.isNotBlank(copyfromwhere)){
			Object objsrc=specService.getFirst(sessionUser,copyfrom, copyfromwhere, null,null);
			commonzSvrUitlsSpec.obj2obj(sessionUser,objsrc, wxrb,copyfrom ,"Wxrb");
			wxrb.setId(null);
		}
		commonzSvrUitlsSpec.newgetBefore(sessionUser,wxrb);
		newgetAfter(sessionUser,wxrb);
		return wxrb;
	}
	/**
	 ...
	 * @throws Exception
	 */
	public Wxrb getOrNew(User sessionUser,Integer id) throws BoException,Exception{
		Wxrb obj=get(sessionUser, id);
		if(obj==null){
			obj = new Wxrb(
				null , //String openid   
				null , //String 父openid   
	 ...
	 * @throws Exception
	 */
	public Wxrb csedit(User sessionUser,Integer id) throws BoException,Exception{
		...
		commonzSvrUitlsSpec.editBefore(sessionUser,wxrb);
		editAfter(sessionUser,wxrb);
		return wxrb;
	}
	////////下面是系列补充与检查
	 ...
	 * @throws Exception
	 */
	public void createAndUpdateBefore(User sessionUser,String simb,Wxrb dbobj,Wxrb obj) throws BoException,Exception{
		if(obj==null)return;
		//create update 共用
		//create 独有
	 ...
	 * @throws Exception
	 */
	public void createAndUpdateAfter(User sessionUser,String simb,Wxrb obj,Wxrb olddbobj) throws BoException,Exception{
		if(obj==null)return;
		//create 独有
		if("create".equals(simb)){
		}
	 ...
	 * @param obj
	 */
	public void indexAfter(User sessionUser,Wxrb obj) throws BoException,Exception{
	 ...
	 * @param obj
	 */
	public void gridBefore(User sessionUser,Wxrb obj) throws BoException,Exception{
		if(obj==null)return;
	 ...
	 * @param list
	 */
	public void gridAfter(User sessionUser,List<Wxrb> list) throws BoException,Exception{
	 ...
	 * @param obj
	 */
	public void gridOneAfter(User sessionUser,Wxrb obj) throws BoException,Exception{
		if(obj==null)return;
	 ...
	 * @param obj
	 */
	public void showAfter(User sessionUser,Wxrb obj) throws BoException,Exception{
		if(obj==null)return;
	 ...
	 * @throws Exception
	 */
	public void newgetAfter(User sessionUser,Wxrb obj) throws BoException,Exception{
		if(obj==null)return;
		 //这里不要查session
	 ...
	 * @param obj
	 */
	public void editAfter(User sessionUser,Wxrb obj) throws BoException,Exception{
		if(obj==null)return;
		addList(sessionUser,obj);
		addString(sessionUser,obj);	
	 ...
	 * @throws Exception
	 */
	public void addObj(User sessionUser,Wxrb obj) throws BoException,Exception{
		if(obj==null)return;
		 //这里不要查session
	 ...
	 * @throws Exception
	 */
	public void addList(User sessionUser,Wxrb obj) throws BoException,Exception{
		if(obj==null)return;
		 //这里不要查session
		//这个以后要去掉的
		obj.setStatusMap((Map)MapDb.getInstance().getMap("Wxrb.Status"));
	 ...
	 * @throws Exception
	 */
	public void addMore(User sessionUser,Wxrb obj) throws BoException,Exception{
		if(obj==null)return;
	 ...
	 * @throws Exception
	 */
	public void addString(User sessionUser,Wxrb obj) throws BoException,Exception{
		if(obj==null)return;
		 //这里不要查session
		String value="";
		value=(String)MapDb.getInstance().getMapString("Wxrb.Status", ""+obj.getStatus());
