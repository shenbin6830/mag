...
import com.zmax.mag.domain.dao.my.UserRepo;


/**
 * 账号信息修改服务
 * 用到我的有：Myisou^UserId^用户参数设置_所属用户,Oplog^UserId^操作日志_操作者,WaTemplateAdd^UserId^模板消息接口之获得模板ID_卖家,Wxmenu^UserId^微信自定义菜单_卖家,WaCustomAudParam^UserId^客服管理之增删改参数_卖家,Wxr^UserId^微信用户关系_平台用户,Wxouser^UserId^微信用户_平台用户,
 * @author zmax
 */
@Service
public class UserService extends BaseService<User> {
	...
	/**扩展版*/
	@Autowired
	BaseRepo<User,Integer> baseRepo;
	@Autowired
	 ...
	 * @return 返回相应的持久化PO实例
	 */
	public User get(User sessionUser,Integer id) {
		if(id==null)
			return null;
		if(id.intValue()==0)
			return null;
		...
		if(obj==null)
			return null;
		if(!permitCheckUtils.checkObj(sessionUser, obj,"User")) 
			return null;
		return obj;
	}
	 ...
	 * @param params 其它参数
	 */
	public User getByKvIfNotExistsSaveKv(User sessionUser,User entity,String keyfield,Object value,String order, Object[] params) {
		if(entity==null)
			return null;
		if(!permitCheckUtils.checkObj(sessionUser, entity,"User")) return entity;
		User obj=getFirst(sessionUser, keyfield+"='"+value.toString()+"'", order, params);
		if(obj==null){
	 ...
	 * @return List
	 */
	public List<User> listfindByIn(User sessionUser,Integer[] ids,String morecondition) {
		String where=SqlUtils.whereFromIds(ids);
		where=SqlUtils.whereAdd(where,permitCheckUtils.addWhere(sessionUser,"User"));
	 ...
	 * @param user
	 */
	public void saveOrUpdate(User sessionUser,User user){
		save(sessionUser,user);
	}
	/**
	 ...
	 * @param entities
	 */
	public void saveOrUpdateAll(User sessionUser,Collection<User> entities) {
		if(entities==null)
			return;
	 ...
	 * @throws Exception
	 */
	public void deleteAll(User sessionUser,List<User> list) throws BoException,Exception{
		for (User obj : list) {
	 ...
	 */
	public void csindex(User sessionUser,User obj,String act) throws BoException,Exception{
		if(StringUtils.equals(act, "nullCreate")){
			if(obj==null)
				throw new Exception("无法创建空对象");
			User dbobj=get(sessionUser, obj.getId());
			if(dbobj==null){
				//创建_写表前操作之自定义，目的是修改obj
				commonzSvrUitlsSpec.createAndUpdateBefore("create",new User(), obj,sessionUser);
	 ...
	 * @return
	 */
	public Grid csgrid(User sessionUser,User search, PageHelper ph,String queryhq) throws BoException,Exception{
	 ...
	 * @throws Exception
	 */
	public void saveCreate(User sessionUser,User oldobj,User obj,String copyfrom,String copyfromwhere) throws BoException,Exception{
		//用于做复制新增的检查
		if(StringUtils.isNotBlank(copyfrom)&&StringUtils.isNotBlank(copyfromwhere)){
			...
			commonzSvrUitlsSpec.obj2objcheck(sessionUser,objsrc, obj ,copyfrom ,"User");
		}		

		//创建_写表前操作之通用
		commonzSvrUitlsSpec.commonCreateBefore(oldobj,obj,"User",sessionUser);
		//创建_写表前操作之本地
		...
		//TODO:lang
		//创建_写表后操作之本地
		createAndUpdateAfter(sessionUser,"create",obj,new User());
		//创建_写表后操作之自定义
		commonzSvrUitlsSpec.createAndUpdateAfter("create",obj,new User(),sessionUser);
	}
	/**
	 * 更新
	 ...
	 * @throws Exception
	 */
	public void updateUpdate(User sessionUser,User obj,Integer id) throws BoException,Exception{
		//数据库表里取出的对象，要把页面对象obj里的内容，放进来进行保存
		User dbobj=get(sessionUser, id);
		if(dbobj==null)
			throw new BoException("数据库中数据找不到");

		//更新_写表前操作之通用，目的是修改obj
		commonzSvrUitlsSpec.commonUpdateBefore(dbobj,obj,"User",sessionUser);
	 ...
	 * @throws Exception
	 */
	public void saveCreateUpdateUpdate(User sessionUser,User oldobj,User obj) throws BoException,Exception{
		if(null==obj.getId()){
	...
	public void updateBatchByIds(User sessionUser,String[] ids,String setfield) throws BoException,Exception{
		String hql= "update User set "+setfield +" where "+StringUtilz.whereIn(ids);
		execute(sessionUser, hql, null);
		commonzSvrUitlsSpec.batchupdateAfter(sessionUser,"User", setfield, ids);
	}
	/**
	 * 根据ID删除一个
	 * @param sessionUser 
	...
	public void deleteOneById(User sessionUser,Integer id) throws BoException,Exception{
		commonzSvrUitlsSpec.deleteByIdBefore(sessionUser,"User", id);
		deleteById(sessionUser,id);
		commonzSvrUitlsSpec.deleteByIdAfter(sessionUser,"User", id);
		//TODO:lang
	}
	/**
	 * 根据IDs删除批量
	 ...
	 * @return
	 */
	public User csshow(User sessionUser,Integer id) throws BoException,Exception{
		User user=get(sessionUser, id);
		showAfter(sessionUser,user);
	 ...
	 * @return
	 */
	public User csnewget(User sessionUser,User user,String copyfrom,String copyfromwhere) throws BoException,Exception{
		if(user==null)
			user=new User();
		//如果是从其它表复制过来的
		if(StringUtils.isNotBlank(copyfrom)&&StringUtils.isNotBlank(copyfromwhere)){
			Object objsrc=specService.getFirst(sessionUser,copyfrom, copyfromwhere, null,null);
			commonzSvrUitlsSpec.obj2obj(sessionUser,objsrc, user,copyfrom ,"User");
			user.setId(null);
		}
		commonzSvrUitlsSpec.newgetBefore(sessionUser,user);
		newgetAfter(sessionUser,user);
		return user;
	}
	/**
	 ...
	 * @throws Exception
	 */
	public User getOrNew(User sessionUser,Integer id) throws BoException,Exception{
		User obj=get(sessionUser, id);
		if(obj==null){
			obj = new User(
				0 , //Integer 状态 default=0  {'0':'正在申请','1':'有效','-1':'申请被拒','-2':'删除'}
	 ...
	 * @throws Exception
	 */
	public User csedit(User sessionUser,Integer id) throws BoException,Exception{
		...
		commonzSvrUitlsSpec.editBefore(sessionUser,user);
		editAfter(sessionUser,user);
		return user;
	}
	////////下面是系列补充与检查
	 ...
	 * @throws Exception
	 */
	public void createAndUpdateBefore(User sessionUser,String simb,User dbobj,User obj) throws BoException,Exception{
		if(obj==null)return;
		//create update 共用
		//create 独有
	 ...
	 * @throws Exception
	 */
	public void createAndUpdateAfter(User sessionUser,String simb,User obj,User olddbobj) throws BoException,Exception{
		if(obj==null)return;
		//create 独有
		if("create".equals(simb)){
		}
	 ...
	 * @param obj
	 */
	public void indexAfter(User sessionUser,User obj) throws BoException,Exception{
	 ...
	 * @param obj
	 */
	public void gridBefore(User sessionUser,User obj) throws BoException,Exception{
		if(obj==null)return;
	 ...
	 * @param list
	 */
	public void gridAfter(User sessionUser,List<User> list) throws BoException,Exception{
	 ...
	 * @param obj
	 */
	public void gridOneAfter(User sessionUser,User obj) throws BoException,Exception{
		if(obj==null)return;
	 ...
	 * @param obj
	 */
	public void showAfter(User sessionUser,User obj) throws BoException,Exception{
		if(obj==null)return;
	 ...
	 * @throws Exception
	 */
	public void newgetAfter(User sessionUser,User obj) throws BoException,Exception{
		if(obj==null)return;
		 //这里不要查session
	 ...
	 * @param obj
	 */
	public void editAfter(User sessionUser,User obj) throws BoException,Exception{
		if(obj==null)return;
		addList(sessionUser,obj);
		addString(sessionUser,obj);	
	 ...
	 * @throws Exception
	 */
	public void addObj(User sessionUser,User obj) throws BoException,Exception{
		if(obj==null)return;
		 //这里不要查session
	 ...
	 * @throws Exception
	 */
	public void addList(User sessionUser,User obj) throws BoException,Exception{
		if(obj==null)return;
		 //这里不要查session
		//这个以后要去掉的
		obj.setStatusMap((Map)MapDb.getInstance().getMap("User.Status"));
		//这个以后要去掉的
		obj.setRoleIdMap((Map)MapDb.getInstance().getMap("User.RoleId"));
	 ...
	 * @throws Exception
	 */
	public void addMore(User sessionUser,User obj) throws BoException,Exception{
		if(obj==null)return;
	 ...
	 * @throws Exception
	 */
	public void addString(User sessionUser,User obj) throws BoException,Exception{
		if(obj==null)return;
		 //这里不要查session
		String value="";
		value=(String)MapDb.getInstance().getMapString("User.Status", ""+obj.getStatus());
		if(StringUtils.isEmpty(value))
			value=""+obj.getStatus();
		obj.setStatusString(value);
		value=(String)MapDb.getInstance().getMapString("User.RoleId", ""+obj.getRoleId());
		if(StringUtils.isEmpty(value))
			value=""+obj.getRoleId();
	 ...
	 * @param user User 父亲
	 */
	public void updateRedundancyToChildren(User sessionUser,User user){
		//Wxr
		wxrService.updateRedundancyFromUser(sessionUser, user);
		commonzSvrUitlsSpec.updateRedundancyToChildren(sessionUser, user);
	}
