...
import com.zmax.mag.domain.dao.my.MyisouRepo;
 ...
 */
@Service
public class MyisouService extends BaseService<Myisou> {
	/**日志实例*/
	...
	/**标准版*/
	@Autowired
	MyisouRepo myisouRepo;
	/**扩展版*/
	@Autowired
	BaseRepo<Myisou,Integer> baseRepo;
	@Autowired
	 ...
	 * @return 返回相应的持久化PO实例
	 */
	public Myisou get(User sessionUser,Integer id) {
		if(id==null)
			return null;
		if(id.intValue()==0)
			return null;
		Myisou obj= myisouRepo.findOne(id);
		if(obj==null)
			return null;
		if(!permitCheckUtils.checkObj(sessionUser, obj,"Myisou")) 
			return null;
		return obj;
	}
	 ...
	 * @param params 其它参数
	 */
	public Myisou getByKvIfNotExistsSaveKv(User sessionUser,Myisou entity,String keyfield,Object value,String order, Object[] params) {
		if(entity==null)
			return null;
		if(!permitCheckUtils.checkObj(sessionUser, entity,"Myisou")) return entity;
		Myisou obj=getFirst(sessionUser, keyfield+"='"+value.toString()+"'", order, params);
		if(obj==null){
	 ...
	 * @return List
	 */
	public List<Myisou> listfindByIn(User sessionUser,Integer[] ids,String morecondition) {
		String where=SqlUtils.whereFromIds(ids);
		where=SqlUtils.whereAdd(where,permitCheckUtils.addWhere(sessionUser,"Myisou"));
		if(StringUtils.isNotBlank(morecondition))
			where=SqlUtils.whereAdd(where, morecondition);
		List<Integer> list=new ArrayList<Integer>();
		return myisouRepo.findAll(list);
	}
	 ...
	 * @param myisou
	 */
	public void save(User sessionUser,Myisou myisou){
		myisouRepo.save(myisou);
	}
	/**
	 * 更新
	 * @param sessionUser
	 * @param myisou
	 */
	public void update(User sessionUser,Myisou myisou){
		myisouRepo.save(myisou);
	}
	/**
	 * 保存，新建或更新，有ID就更新，否则新建
	 ...
	 * @param myisou
	 */
	public void saveOrUpdate(User sessionUser,Myisou myisou){
		save(sessionUser,myisou);
	}
	/**
	 ...
	 * @param entities
	 */
	public void saveOrUpdateAll(User sessionUser,Collection<Myisou> entities) {
		if(entities==null)
			return;
		for (Myisou myisou : entities) {
			save(sessionUser,myisou);
		}
	}
	/**
	 ...
	 * @throws Exception
	 */
	public void deleteAll(User sessionUser,List<Myisou> list) throws BoException,Exception{
		for (Myisou obj : list) {
	 ...
	 */
	public void csindex(User sessionUser,Myisou obj,String act) throws BoException,Exception{
		if(StringUtils.equals(act, "nullCreate")){
			if(obj==null)
				throw new Exception("无法创建空对象");
			Myisou dbobj=get(sessionUser, obj.getId());
			if(dbobj==null){
				//创建_写表前操作之自定义，目的是修改obj
				commonzSvrUitlsSpec.createAndUpdateBefore("create",new Myisou(), obj,sessionUser);
	 ...
	 * @return
	 */
	public Grid csgrid(User sessionUser,Myisou search, PageHelper ph,String queryhq) throws BoException,Exception{
	 ...
	 * @throws Exception
	 */
	public void saveCreate(User sessionUser,Myisou oldobj,Myisou obj,String copyfrom,String copyfromwhere) throws BoException,Exception{
		//用于做复制新增的检查
		if(StringUtils.isNotBlank(copyfrom)&&StringUtils.isNotBlank(copyfromwhere)){
			...
			commonzSvrUitlsSpec.obj2objcheck(sessionUser,objsrc, obj ,copyfrom ,"Myisou");
		}		

		//创建_写表前操作之通用
		commonzSvrUitlsSpec.commonCreateBefore(oldobj,obj,"Myisou",sessionUser);
		//创建_写表前操作之本地
		...
		//TODO:lang
		//创建_写表后操作之本地
		createAndUpdateAfter(sessionUser,"create",obj,new Myisou());
		//创建_写表后操作之自定义
		commonzSvrUitlsSpec.createAndUpdateAfter("create",obj,new Myisou(),sessionUser);
	}
	/**
	 * 更新
	 ...
	 * @throws Exception
	 */
	public void updateUpdate(User sessionUser,Myisou obj,Integer id) throws BoException,Exception{
		//数据库表里取出的对象，要把页面对象obj里的内容，放进来进行保存
		Myisou dbobj=get(sessionUser, id);
		if(dbobj==null)
			throw new BoException("数据库中数据找不到");

		//更新_写表前操作之通用，目的是修改obj
		commonzSvrUitlsSpec.commonUpdateBefore(dbobj,obj,"Myisou",sessionUser);
	 ...
	 * @throws Exception
	 */
	public void saveCreateUpdateUpdate(User sessionUser,Myisou oldobj,Myisou obj) throws BoException,Exception{
		if(null==obj.getId()){
	...
	public void updateBatchByIds(User sessionUser,String[] ids,String setfield) throws BoException,Exception{
		String hql= "update Myisou set "+setfield +" where "+StringUtilz.whereIn(ids);
		execute(sessionUser, hql, null);
		commonzSvrUitlsSpec.batchupdateAfter(sessionUser,"Myisou", setfield, ids);
	}
	/**
	 * 根据ID删除一个
	 * @param sessionUser 
	...
	public void deleteOneById(User sessionUser,Integer id) throws BoException,Exception{
		commonzSvrUitlsSpec.deleteByIdBefore(sessionUser,"Myisou", id);
		deleteById(sessionUser,id);
		commonzSvrUitlsSpec.deleteByIdAfter(sessionUser,"Myisou", id);
		//TODO:lang
	}
	/**
	 * 根据IDs删除批量
	 ...
	 * @return
	 */
	public Myisou csshow(User sessionUser,Integer id) throws BoException,Exception{
		Myisou myisou=get(sessionUser, id);
		showAfter(sessionUser,myisou);
		return (myisou==null)?new Myisou():myisou;
	}
	/**
	 * 新建GET
	 * @param sessionUser 
	 ...
	 * @return
	 */
	public Myisou csnewget(User sessionUser,Myisou myisou,String copyfrom,String copyfromwhere) throws BoException,Exception{
		if(myisou==null)
			myisou=new Myisou();
		//如果是从其它表复制过来的
		if(StringUtils.isNotBlank(copyfrom)&&StringUtils.isNotBlank(copyfromwhere)){
			Object objsrc=specService.getFirst(sessionUser,copyfrom, copyfromwhere, null,null);
			commonzSvrUitlsSpec.obj2obj(sessionUser,objsrc, myisou,copyfrom ,"Myisou");
			myisou.setId(null);
		}
		commonzSvrUitlsSpec.newgetBefore(sessionUser,myisou);
		newgetAfter(sessionUser,myisou);
		return myisou;
	}
	/**
	 ...
	 * @throws Exception
	 */
	public Myisou getOrNew(User sessionUser,Integer id) throws BoException,Exception{
		Myisou obj=get(sessionUser, id);
		if(obj==null){
			obj = new Myisou(
				null , //Integer 所属用户   
				0 , //Integer 排列顺序 default=0  
				null , //String 备注说明   
				null , //String 关键字  all.字段名或[表名].字段名 
				null , //String 保存值  数据库保存这个 
				null , //String 显示  页面显示这个 
				null
			);
			obj.setId(id);
			save(sessionUser,obj);
	 ...
	 * @throws Exception
	 */
	public Myisou csedit(User sessionUser,Integer id) throws BoException,Exception{
		...
		commonzSvrUitlsSpec.editBefore(sessionUser,myisou);
		editAfter(sessionUser,myisou);
		return myisou;
	}
	////////下面是系列补充与检查
	 ...
	 * @throws Exception
	 */
	public void createAndUpdateBefore(User sessionUser,String simb,Myisou dbobj,Myisou obj) throws BoException,Exception{
		if(obj==null)return;
		//create update 共用
		//create 独有
	 ...
	 * @throws Exception
	 */
	public void createAndUpdateAfter(User sessionUser,String simb,Myisou obj,Myisou olddbobj) throws BoException,Exception{
		if(obj==null)return;
		//create 独有
		if("create".equals(simb)){
		}
	 ...
	 * @param obj
	 */
	public void indexAfter(User sessionUser,Myisou obj) throws BoException,Exception{
	 ...
	 * @param obj
	 */
	public void gridBefore(User sessionUser,Myisou obj) throws BoException,Exception{
		if(obj==null)return;
	 ...
	 * @param list
	 */
	public void gridAfter(User sessionUser,List<Myisou> list) throws BoException,Exception{
	 ...
	 * @param obj
	 */
	public void gridOneAfter(User sessionUser,Myisou obj) throws BoException,Exception{
		if(obj==null)return;
	 ...
	 * @param obj
	 */
	public void showAfter(User sessionUser,Myisou obj) throws BoException,Exception{
		if(obj==null)return;
	 ...
	 * @throws Exception
	 */
	public void newgetAfter(User sessionUser,Myisou obj) throws BoException,Exception{
		if(obj==null)return;
		 //这里不要查session
	 ...
	 * @param obj
	 */
	public void editAfter(User sessionUser,Myisou obj) throws BoException,Exception{
		if(obj==null)return;
		addList(sessionUser,obj);
		addString(sessionUser,obj);	
	 ...
	 * @throws Exception
	 */
	public void addObj(User sessionUser,Myisou obj) throws BoException,Exception{
		if(obj==null)return;
		 //这里不要查session
	 ...
	 * @throws Exception
	 */
	public void addList(User sessionUser,Myisou obj) throws BoException,Exception{
		if(obj==null)return;
		 //这里不要查session
	 ...
	 * @throws Exception
	 */
	public void addMore(User sessionUser,Myisou obj) throws BoException,Exception{
		if(obj==null)return;
	 ...
	 * @throws Exception
	 */
	public void addString(User sessionUser,Myisou obj) throws BoException,Exception{
		if(obj==null)return;
		 //这里不要查session
		String value="";
