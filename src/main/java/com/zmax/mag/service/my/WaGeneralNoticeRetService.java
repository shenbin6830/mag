...
import com.zmax.mag.domain.dao.my.WaGeneralNoticeRetRepo;
...
public class WaGeneralNoticeRetService extends BaseService<WaGeneralNoticeRet> {
	...
	/**标准版*/
	@Autowired
	WaGeneralNoticeRetRepo waGeneralNoticeRetRepo;
	/**扩展版*/
	@Autowired
	BaseRepo<WaGeneralNoticeRet,Integer> baseRepo;
	@Autowired
	 ...
	 * @return 返回相应的持久化PO实例
	 */
	public WaGeneralNoticeRet get(User sessionUser,Integer id) {
		if(id==null)
			return null;
		if(id.intValue()==0)
			return null;
		WaGeneralNoticeRet obj= waGeneralNoticeRetRepo.findOne(id);
		if(obj==null)
			return null;
		if(!permitCheckUtils.checkObj(sessionUser, obj,"WaGeneralNoticeRet")) 
			return null;
		return obj;
	}
	 ...
	 * @param params 其它参数
	 */
	public WaGeneralNoticeRet getByKvIfNotExistsSaveKv(User sessionUser,WaGeneralNoticeRet entity,String keyfield,Object value,String order, Object[] params) {
		if(entity==null)
			return null;
		if(!permitCheckUtils.checkObj(sessionUser, entity,"WaGeneralNoticeRet")) return entity;
		WaGeneralNoticeRet obj=getFirst(sessionUser, keyfield+"='"+value.toString()+"'", order, params);
		if(obj==null){
	 ...
	 * @return List
	 */
	public List<WaGeneralNoticeRet> listfindByIn(User sessionUser,Integer[] ids,String morecondition) {
		String where=SqlUtils.whereFromIds(ids);
		where=SqlUtils.whereAdd(where,permitCheckUtils.addWhere(sessionUser,"WaGeneralNoticeRet"));
		if(StringUtils.isNotBlank(morecondition))
			where=SqlUtils.whereAdd(where, morecondition);
		List<Integer> list=new ArrayList<Integer>();
		return waGeneralNoticeRetRepo.findAll(list);
	}
	 ...
	 */
	public void save(User sessionUser,WaGeneralNoticeRet waGeneralNoticeRet){
		waGeneralNoticeRetRepo.save(waGeneralNoticeRet);
	}
	/**
	 * 更新
	 * @param sessionUser
	 * @param waGeneralNoticeRet
	 */
	public void update(User sessionUser,WaGeneralNoticeRet waGeneralNoticeRet){
		waGeneralNoticeRetRepo.save(waGeneralNoticeRet);
	}
	/**
	 * 保存，新建或更新，有ID就更新，否则新建
	 ...
	 * @param waGeneralNoticeRet
	 */
	public void saveOrUpdate(User sessionUser,WaGeneralNoticeRet waGeneralNoticeRet){
		save(sessionUser,waGeneralNoticeRet);
	}
	/**
	 ...
	 * @param entities
	 */
	public void saveOrUpdateAll(User sessionUser,Collection<WaGeneralNoticeRet> entities) {
		if(entities==null)
			return;
		for (WaGeneralNoticeRet waGeneralNoticeRet : entities) {
			save(sessionUser,waGeneralNoticeRet);
		}
	}
	/**
	...
	public void deleteById(User sessionUser,Integer id){
		waGeneralNoticeRetRepo.delete(id);
	}
	 ...
	 * @throws Exception
	 */
	public void deleteAll(User sessionUser,List<WaGeneralNoticeRet> list) throws BoException,Exception{
		for (WaGeneralNoticeRet obj : list) {
	 ...
	 */
	public void csindex(User sessionUser,WaGeneralNoticeRet obj,String act) throws BoException,Exception{
	 ...
	 * @return
	 */
	public Grid csgrid(User sessionUser,WaGeneralNoticeRet search, PageHelper ph,String queryhq) throws BoException,Exception{
	 ...
	 * @throws Exception
	 */
	public void saveCreate(User sessionUser,WaGeneralNoticeRet oldobj,WaGeneralNoticeRet obj,String copyfrom,String copyfromwhere) throws BoException,Exception{
		//用于做复制新增的检查
		...
		//创建_写表前操作之通用
		commonzSvrUitlsSpec.commonCreateBefore(oldobj,obj,"WaGeneralNoticeRet",sessionUser);
		//创建_写表前操作之本地
	 ...
	 * @throws Exception
	 */
	public void updateUpdate(User sessionUser,WaGeneralNoticeRet obj,Integer id) throws BoException,Exception{
		...
		commonzSvrUitlsSpec.commonUpdateBefore(dbobj,obj,"WaGeneralNoticeRet",sessionUser);
	 ...
	 * @throws Exception
	 */
	public void saveCreateUpdateUpdate(User sessionUser,WaGeneralNoticeRet oldobj,WaGeneralNoticeRet obj) throws BoException,Exception{
		if(null==obj.getId()){
	 ...
	 * @return
	 */
	public WaGeneralNoticeRet csshow(User sessionUser,Integer id) throws BoException,Exception{
	 ...
	 * @return
	 */
	public WaGeneralNoticeRet csnewget(User sessionUser,WaGeneralNoticeRet waGeneralNoticeRet,String copyfrom,String copyfromwhere) throws BoException,Exception{
	 ...
	 * @throws Exception
	 */
	public WaGeneralNoticeRet getOrNew(User sessionUser,Integer id) throws BoException,Exception{
		WaGeneralNoticeRet obj=get(sessionUser, id);
		if(obj==null){
			obj = new WaGeneralNoticeRet(
				null , //String 返回状态码  SUCCESS/FAIL 
				null , //String 返回信息  返回信息，如非空，为错误原因 签名失败 参数格式校验错误 
				...
				null , //String 签名  签名 
				null , //String 业务结果  SUCCESS/FAIL 
				null , //String 错误代码  详细参见第6节错误列表 
				null , //String 错误代码描述  错误返回的信息描述 
				null , //String 用户标识  用户在商户appid下的唯一标识 
				null , //String 是否关注公众账号  用户是否关注公众账号，Y-关注，N-未关注，仅在公众账号类型支付有效 
				null , //String 交易类型  JSAPI、NATIVE、APP 
				null , //String 付款银行  银行类型，采用字符串类型的银行标识，银行类型见附表 
				null , //Integer 总金额  订单总金额，单位为分 
				null , //String 货币种类  货币类型，符合ISO 4217标准的三位字母代码，默认人民币：CNY， 
				null , //Integer 现金支付金额  现金支付金额订单现金支付金额 
				null , //String 现金支付货币类型  货币类型，符合ISO 4217标准的三位字母代码，默认人民币：CNY 
				null , //Integer 代金券或立减优惠金额  代金券或立减优惠金额<=订单总金额，订单总金额-代金券或立减优惠金额=现金支付金额 
				null , //Integer 代金券或立减优惠使用数量  代金券或立减优惠使用数量 
				null , //String 微信支付订单号  微信支付订单号 
				null , //String 商户订单号  商户系统的订单号，与请求一致。 
				null , //String 商家数据包  商家数据包，原样返回 
				null , //String 支付完成时间  支付完成时间，格式为yyyyMMddHHmmss，如2009年12月25日9点10分10秒表示为20091225091010。 
				null
			);
			obj.setId(id);
			save(sessionUser,obj);
	 ...
	 * @throws Exception
	 */
	public WaGeneralNoticeRet csedit(User sessionUser,Integer id) throws BoException,Exception{
