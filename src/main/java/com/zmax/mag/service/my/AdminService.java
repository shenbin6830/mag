...
import com.zmax.mag.domain.dao.my.AdminRepo;
...
public class AdminService extends BaseService<Admin> {
	...
	/**扩展版*/
	@Autowired
	BaseRepo<Admin,Integer> baseRepo;
	@Autowired
	 ...
	 * @return 返回相应的持久化PO实例
	 */
	public Admin get(User sessionUser,Integer id) {
		if(id==null)
			return null;
		if(id.intValue()==0)
			return null;
		Admin obj= adminRepo.findOne(id);
		if(obj==null)
			return null;
		if(!permitCheckUtils.checkObj(sessionUser, obj,"Admin")) 
			return null;
		return obj;
	}
	 ...
	 * @param params 其它参数
	 */
	public Admin getByKvIfNotExistsSaveKv(User sessionUser,Admin entity,String keyfield,Object value,String order, Object[] params) {
		if(entity==null)
			return null;
		if(!permitCheckUtils.checkObj(sessionUser, entity,"Admin")) return entity;
		Admin obj=getFirst(sessionUser, keyfield+"='"+value.toString()+"'", order, params);
		if(obj==null){
	 ...
	 * @return List
	 */
	public List<Admin> listfindByIn(User sessionUser,Integer[] ids,String morecondition) {
		String where=SqlUtils.whereFromIds(ids);
		where=SqlUtils.whereAdd(where,permitCheckUtils.addWhere(sessionUser,"Admin"));
		if(StringUtils.isNotBlank(morecondition))
			where=SqlUtils.whereAdd(where, morecondition);
		List<Integer> list=new ArrayList<Integer>();
		return adminRepo.findAll(list);
	}
	 ...
	 * @param admin
	 */
	public void save(User sessionUser,Admin admin){
	 ...
	 * @param admin
	 */
	public void update(User sessionUser,Admin admin){
		adminRepo.save(admin);
	}
	/**
	 * 保存，新建或更新，有ID就更新，否则新建
	 ...
	 * @param admin
	 */
	public void saveOrUpdate(User sessionUser,Admin admin){
		save(sessionUser,admin);
	}
	/**
	 ...
	 * @param entities
	 */
	public void saveOrUpdateAll(User sessionUser,Collection<Admin> entities) {
		if(entities==null)
			return;
	 ...
	 * @throws Exception
	 */
	public void deleteAll(User sessionUser,List<Admin> list) throws BoException,Exception{
		for (Admin obj : list) {
	 ...
	 */
	public void csindex(User sessionUser,Admin obj,String act) throws BoException,Exception{
		if(StringUtils.equals(act, "nullCreate")){
			if(obj==null)
				throw new Exception("无法创建空对象");
			Admin dbobj=get(sessionUser, obj.getId());
			if(dbobj==null){
				//创建_写表前操作之自定义，目的是修改obj
				commonzSvrUitlsSpec.createAndUpdateBefore("create",new Admin(), obj,sessionUser);
	 ...
	 * @return
	 */
	public Grid csgrid(User sessionUser,Admin search, PageHelper ph,String queryhq) throws BoException,Exception{
	 ...
	 * @throws Exception
	 */
	public void saveCreate(User sessionUser,Admin oldobj,Admin obj,String copyfrom,String copyfromwhere) throws BoException,Exception{
		//用于做复制新增的检查
		if(StringUtils.isNotBlank(copyfrom)&&StringUtils.isNotBlank(copyfromwhere)){
			...
			commonzSvrUitlsSpec.obj2objcheck(sessionUser,objsrc, obj ,copyfrom ,"Admin");
		}		

		//创建_写表前操作之通用
		commonzSvrUitlsSpec.commonCreateBefore(oldobj,obj,"Admin",sessionUser);
		//创建_写表前操作之本地
		...
		//TODO:lang
		//创建_写表后操作之本地
		createAndUpdateAfter(sessionUser,"create",obj,new Admin());
		//创建_写表后操作之自定义
		commonzSvrUitlsSpec.createAndUpdateAfter("create",obj,new Admin(),sessionUser);
	}
	/**
	 * 更新
	 ...
	 * @throws Exception
	 */
	public void updateUpdate(User sessionUser,Admin obj,Integer id) throws BoException,Exception{
		//数据库表里取出的对象，要把页面对象obj里的内容，放进来进行保存
		Admin dbobj=get(sessionUser, id);
		if(dbobj==null)
			throw new BoException("数据库中数据找不到");

		//更新_写表前操作之通用，目的是修改obj
		commonzSvrUitlsSpec.commonUpdateBefore(dbobj,obj,"Admin",sessionUser);
	 ...
	 * @throws Exception
	 */
	public void saveCreateUpdateUpdate(User sessionUser,Admin oldobj,Admin obj) throws BoException,Exception{
		if(null==obj.getId()){
	...
	public void updateBatchByIds(User sessionUser,String[] ids,String setfield) throws BoException,Exception{
		String hql= "update Admin set "+setfield +" where "+StringUtilz.whereIn(ids);
		execute(sessionUser, hql, null);
		commonzSvrUitlsSpec.batchupdateAfter(sessionUser,"Admin", setfield, ids);
	}
	/**
	 * 根据ID删除一个
	 * @param sessionUser 
	...
	public void deleteOneById(User sessionUser,Integer id) throws BoException,Exception{
		commonzSvrUitlsSpec.deleteByIdBefore(sessionUser,"Admin", id);
		deleteById(sessionUser,id);
		commonzSvrUitlsSpec.deleteByIdAfter(sessionUser,"Admin", id);
		//TODO:lang
	}
	/**
	 * 根据IDs删除批量
	 ...
	 * @return
	 */
	public Admin csshow(User sessionUser,Integer id) throws BoException,Exception{
		Admin admin=get(sessionUser, id);
		showAfter(sessionUser,admin);
	 ...
	 * @return
	 */
	public Admin csnewget(User sessionUser,Admin admin,String copyfrom,String copyfromwhere) throws BoException,Exception{
		if(admin==null)
			admin=new Admin();
		//如果是从其它表复制过来的
		if(StringUtils.isNotBlank(copyfrom)&&StringUtils.isNotBlank(copyfromwhere)){
			Object objsrc=specService.getFirst(sessionUser,copyfrom, copyfromwhere, null,null);
			commonzSvrUitlsSpec.obj2obj(sessionUser,objsrc, admin,copyfrom ,"Admin");
			admin.setId(null);
		}
		commonzSvrUitlsSpec.newgetBefore(sessionUser,admin);
		newgetAfter(sessionUser,admin);
		return admin;
	}
	/**
	 ...
	 * @throws Exception
	 */
	public Admin getOrNew(User sessionUser,Integer id) throws BoException,Exception{
		Admin obj=get(sessionUser, id);
		if(obj==null){
			obj = new Admin(
				null , //String 昵称   
				null
	 ...
	 * @throws Exception
	 */
	public Admin csedit(User sessionUser,Integer id) throws BoException,Exception{
		...
		commonzSvrUitlsSpec.editBefore(sessionUser,admin);
		editAfter(sessionUser,admin);
		return admin;
	}
	////////下面是系列补充与检查
	 ...
	 * @throws Exception
	 */
	public void createAndUpdateBefore(User sessionUser,String simb,Admin dbobj,Admin obj) throws BoException,Exception{
		if(obj==null)return;
		//create update 共用
		//create 独有
	 ...
	 * @throws Exception
	 */
	public void createAndUpdateAfter(User sessionUser,String simb,Admin obj,Admin olddbobj) throws BoException,Exception{
		if(obj==null)return;
		//create 独有
		if("create".equals(simb)){
		}
	 ...
	 * @param obj
	 */
	public void indexAfter(User sessionUser,Admin obj) throws BoException,Exception{
	 ...
	 * @param obj
	 */
	public void gridBefore(User sessionUser,Admin obj) throws BoException,Exception{
		if(obj==null)return;
	 ...
	 * @param list
	 */
	public void gridAfter(User sessionUser,List<Admin> list) throws BoException,Exception{
	 ...
	 * @param obj
	 */
	public void gridOneAfter(User sessionUser,Admin obj) throws BoException,Exception{
		if(obj==null)return;
	 ...
	 * @param obj
	 */
	public void showAfter(User sessionUser,Admin obj) throws BoException,Exception{
		if(obj==null)return;
	 ...
	 * @throws Exception
	 */
	public void newgetAfter(User sessionUser,Admin obj) throws BoException,Exception{
		if(obj==null)return;
		 //这里不要查session
	 ...
	 * @param obj
	 */
	public void editAfter(User sessionUser,Admin obj) throws BoException,Exception{
		if(obj==null)return;
		addList(sessionUser,obj);
		addString(sessionUser,obj);	
	 ...
	 * @throws Exception
	 */
	public void addObj(User sessionUser,Admin obj) throws BoException,Exception{
		if(obj==null)return;
		 //这里不要查session
	 ...
	 * @throws Exception
	 */
	public void addList(User sessionUser,Admin obj) throws BoException,Exception{
		if(obj==null)return;
		 //这里不要查session
	 ...
	 * @throws Exception
	 */
	public void addMore(User sessionUser,Admin obj) throws BoException,Exception{
		if(obj==null)return;
	 ...
	 * @throws Exception
	 */
	public void addString(User sessionUser,Admin obj) throws BoException,Exception{
		if(obj==null)return;
		 //这里不要查session
		String value="";
