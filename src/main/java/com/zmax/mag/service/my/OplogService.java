...
import com.zmax.mag.domain.dao.my.OplogRepo;
...
public class OplogService extends BaseService<Oplog> {
	...
	/**扩展版*/
	@Autowired
	BaseRepo<Oplog,Integer> baseRepo;
	@Autowired
	 ...
	 * @return 返回相应的持久化PO实例
	 */
	public Oplog get(User sessionUser,Integer id) {
		if(id==null)
			return null;
		if(id.intValue()==0)
			return null;
		Oplog obj= oplogRepo.findOne(id);
		if(obj==null)
			return null;
		if(!permitCheckUtils.checkObj(sessionUser, obj,"Oplog")) 
			return null;
		return obj;
	}
	 ...
	 * @param params 其它参数
	 */
	public Oplog getByKvIfNotExistsSaveKv(User sessionUser,Oplog entity,String keyfield,Object value,String order, Object[] params) {
		if(entity==null)
			return null;
		if(!permitCheckUtils.checkObj(sessionUser, entity,"Oplog")) return entity;
		Oplog obj=getFirst(sessionUser, keyfield+"='"+value.toString()+"'", order, params);
		if(obj==null){
	 ...
	 * @return List
	 */
	public List<Oplog> listfindByIn(User sessionUser,Integer[] ids,String morecondition) {
		String where=SqlUtils.whereFromIds(ids);
		where=SqlUtils.whereAdd(where,permitCheckUtils.addWhere(sessionUser,"Oplog"));
		if(StringUtils.isNotBlank(morecondition))
			where=SqlUtils.whereAdd(where, morecondition);
		List<Integer> list=new ArrayList<Integer>();
		return oplogRepo.findAll(list);
	}
	 ...
	 * @param oplog
	 */
	public void save(User sessionUser,Oplog oplog){
		oplogRepo.save(oplog);
	}
	/**
	 * 更新
	 * @param sessionUser
	 * @param oplog
	 */
	public void update(User sessionUser,Oplog oplog){
		oplogRepo.save(oplog);
	}
	/**
	 * 保存，新建或更新，有ID就更新，否则新建
	 ...
	 * @param oplog
	 */
	public void saveOrUpdate(User sessionUser,Oplog oplog){
		save(sessionUser,oplog);
	}
	/**
	 ...
	 * @param entities
	 */
	public void saveOrUpdateAll(User sessionUser,Collection<Oplog> entities) {
		if(entities==null)
			return;
		for (Oplog oplog : entities) {
			save(sessionUser,oplog);
		}
	}
	/**
	 ...
	 * @throws Exception
	 */
	public void deleteAll(User sessionUser,List<Oplog> list) throws BoException,Exception{
		for (Oplog obj : list) {
	 ...
	 */
	public void csindex(User sessionUser,Oplog obj,String act) throws BoException,Exception{
		if(StringUtils.equals(act, "nullCreate")){
			if(obj==null)
				throw new Exception("无法创建空对象");
			Oplog dbobj=get(sessionUser, obj.getId());
			if(dbobj==null){
				//创建_写表前操作之自定义，目的是修改obj
				commonzSvrUitlsSpec.createAndUpdateBefore("create",new Oplog(), obj,sessionUser);
	 ...
	 * @return
	 */
	public Grid csgrid(User sessionUser,Oplog search, PageHelper ph,String queryhq) throws BoException,Exception{
	 ...
	 * @throws Exception
	 */
	public void saveCreate(User sessionUser,Oplog oldobj,Oplog obj,String copyfrom,String copyfromwhere) throws BoException,Exception{
		//用于做复制新增的检查
		if(StringUtils.isNotBlank(copyfrom)&&StringUtils.isNotBlank(copyfromwhere)){
			...
			commonzSvrUitlsSpec.obj2objcheck(sessionUser,objsrc, obj ,copyfrom ,"Oplog");
		}		

		//创建_写表前操作之通用
		commonzSvrUitlsSpec.commonCreateBefore(oldobj,obj,"Oplog",sessionUser);
		//创建_写表前操作之本地
		...
		//TODO:lang
		//创建_写表后操作之本地
		createAndUpdateAfter(sessionUser,"create",obj,new Oplog());
		//创建_写表后操作之自定义
		commonzSvrUitlsSpec.createAndUpdateAfter("create",obj,new Oplog(),sessionUser);
	}
	/**
	 * 更新
	 ...
	 * @throws Exception
	 */
	public void updateUpdate(User sessionUser,Oplog obj,Integer id) throws BoException,Exception{
		//数据库表里取出的对象，要把页面对象obj里的内容，放进来进行保存
		Oplog dbobj=get(sessionUser, id);
		if(dbobj==null)
			throw new BoException("数据库中数据找不到");

		//更新_写表前操作之通用，目的是修改obj
		commonzSvrUitlsSpec.commonUpdateBefore(dbobj,obj,"Oplog",sessionUser);
	 ...
	 * @throws Exception
	 */
	public void saveCreateUpdateUpdate(User sessionUser,Oplog oldobj,Oplog obj) throws BoException,Exception{
		if(null==obj.getId()){
	...
	public void updateBatchByIds(User sessionUser,String[] ids,String setfield) throws BoException,Exception{
		String hql= "update Oplog set "+setfield +" where "+StringUtilz.whereIn(ids);
		execute(sessionUser, hql, null);
		commonzSvrUitlsSpec.batchupdateAfter(sessionUser,"Oplog", setfield, ids);
	}
	/**
	 * 根据ID删除一个
	 * @param sessionUser 
	...
	public void deleteOneById(User sessionUser,Integer id) throws BoException,Exception{
		commonzSvrUitlsSpec.deleteByIdBefore(sessionUser,"Oplog", id);
		deleteById(sessionUser,id);
		commonzSvrUitlsSpec.deleteByIdAfter(sessionUser,"Oplog", id);
		//TODO:lang
	}
	/**
	 * 根据IDs删除批量
	 ...
	 * @return
	 */
	public Oplog csshow(User sessionUser,Integer id) throws BoException,Exception{
		Oplog oplog=get(sessionUser, id);
		showAfter(sessionUser,oplog);
		return (oplog==null)?new Oplog():oplog;
	}
	/**
	 * 新建GET
	 * @param sessionUser 
	 ...
	 * @return
	 */
	public Oplog csnewget(User sessionUser,Oplog oplog,String copyfrom,String copyfromwhere) throws BoException,Exception{
		if(oplog==null)
			oplog=new Oplog();
		//如果是从其它表复制过来的
		if(StringUtils.isNotBlank(copyfrom)&&StringUtils.isNotBlank(copyfromwhere)){
			Object objsrc=specService.getFirst(sessionUser,copyfrom, copyfromwhere, null,null);
			commonzSvrUitlsSpec.obj2obj(sessionUser,objsrc, oplog,copyfrom ,"Oplog");
			oplog.setId(null);
		}
		commonzSvrUitlsSpec.newgetBefore(sessionUser,oplog);
		newgetAfter(sessionUser,oplog);
		return oplog;
	}
	/**
	 ...
	 * @throws Exception
	 */
	public Oplog getOrNew(User sessionUser,Integer id) throws BoException,Exception{
		Oplog obj=get(sessionUser, id);
		if(obj==null){
			obj = new Oplog(
				null , //String ip   
				null , //Integer 操作者   
				0 , //Integer 操作类型 default=0  {'0':'其它','1':'增加','2':'删除','3':'修改','4':'查询','5':'列表','6':'其它查询','7':'其它修改','8':'批量修改'}
				null , //String 操作表   
				null , //String 操作表id   
				null , //String 原始操作   
				null , //String 备注   
	 ...
	 * @throws Exception
	 */
	public Oplog csedit(User sessionUser,Integer id) throws BoException,Exception{
		...
		commonzSvrUitlsSpec.editBefore(sessionUser,oplog);
		editAfter(sessionUser,oplog);
		return oplog;
	}
	////////下面是系列补充与检查
	 ...
	 * @throws Exception
	 */
	public void createAndUpdateBefore(User sessionUser,String simb,Oplog dbobj,Oplog obj) throws BoException,Exception{
		if(obj==null)return;
		//create update 共用
		//create 独有
	 ...
	 * @throws Exception
	 */
	public void createAndUpdateAfter(User sessionUser,String simb,Oplog obj,Oplog olddbobj) throws BoException,Exception{
		if(obj==null)return;
		//create 独有
		if("create".equals(simb)){
		}
	 ...
	 * @param obj
	 */
	public void indexAfter(User sessionUser,Oplog obj) throws BoException,Exception{
	 ...
	 * @param obj
	 */
	public void gridBefore(User sessionUser,Oplog obj) throws BoException,Exception{
		if(obj==null)return;
	 ...
	 * @param list
	 */
	public void gridAfter(User sessionUser,List<Oplog> list) throws BoException,Exception{
	 ...
	 * @param obj
	 */
	public void gridOneAfter(User sessionUser,Oplog obj) throws BoException,Exception{
		if(obj==null)return;
	 ...
	 * @param obj
	 */
	public void showAfter(User sessionUser,Oplog obj) throws BoException,Exception{
		if(obj==null)return;
	 ...
	 * @throws Exception
	 */
	public void newgetAfter(User sessionUser,Oplog obj) throws BoException,Exception{
		if(obj==null)return;
		 //这里不要查session
	 ...
	 * @param obj
	 */
	public void editAfter(User sessionUser,Oplog obj) throws BoException,Exception{
		if(obj==null)return;
		addList(sessionUser,obj);
		addString(sessionUser,obj);	
	 ...
	 * @throws Exception
	 */
	public void addObj(User sessionUser,Oplog obj) throws BoException,Exception{
		if(obj==null)return;
		 //这里不要查session
	 ...
	 * @throws Exception
	 */
	public void addList(User sessionUser,Oplog obj) throws BoException,Exception{
		if(obj==null)return;
		 //这里不要查session
		//这个以后要去掉的
		obj.setItypeMap((Map)MapDb.getInstance().getMap("Oplog.Itype"));
	 ...
	 * @throws Exception
	 */
	public void addMore(User sessionUser,Oplog obj) throws BoException,Exception{
		if(obj==null)return;
	 ...
	 * @throws Exception
	 */
	public void addString(User sessionUser,Oplog obj) throws BoException,Exception{
		if(obj==null)return;
		 //这里不要查session
		String value="";
		...
		value=(String)MapDb.getInstance().getMapString("Oplog.Itype", ""+obj.getItype());
