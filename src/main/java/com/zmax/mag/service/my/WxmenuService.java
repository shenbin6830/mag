...
import com.zmax.mag.domain.dao.my.WxmenuRepo;
...
public class WxmenuService extends BaseService<Wxmenu> {
	...
	/**标准版*/
	@Autowired
	WxmenuRepo wxmenuRepo;
	/**扩展版*/
	@Autowired
	BaseRepo<Wxmenu,Integer> baseRepo;
	@Autowired
	 ...
	 * @return 返回相应的持久化PO实例
	 */
	public Wxmenu get(User sessionUser,Integer id) {
		if(id==null)
			return null;
		if(id.intValue()==0)
			return null;
		Wxmenu obj= wxmenuRepo.findOne(id);
		if(obj==null)
			return null;
		if(!permitCheckUtils.checkObj(sessionUser, obj,"Wxmenu")) 
			return null;
		return obj;
	}
	 ...
	 * @param params 其它参数
	 */
	public Wxmenu getByKvIfNotExistsSaveKv(User sessionUser,Wxmenu entity,String keyfield,Object value,String order, Object[] params) {
		if(entity==null)
			return null;
		if(!permitCheckUtils.checkObj(sessionUser, entity,"Wxmenu")) return entity;
		Wxmenu obj=getFirst(sessionUser, keyfield+"='"+value.toString()+"'", order, params);
		if(obj==null){
	 ...
	 * @return List
	 */
	public List<Wxmenu> listfindByIn(User sessionUser,Integer[] ids,String morecondition) {
		String where=SqlUtils.whereFromIds(ids);
		where=SqlUtils.whereAdd(where,permitCheckUtils.addWhere(sessionUser,"Wxmenu"));
		if(StringUtils.isNotBlank(morecondition))
			where=SqlUtils.whereAdd(where, morecondition);
		List<Integer> list=new ArrayList<Integer>();
		return wxmenuRepo.findAll(list);
	}
	 ...
	 * @param wxmenu
	 */
	public void save(User sessionUser,Wxmenu wxmenu){
		wxmenuRepo.save(wxmenu);
	}
	/**
	 * 更新
	 * @param sessionUser
	 * @param wxmenu
	 */
	public void update(User sessionUser,Wxmenu wxmenu){
		wxmenuRepo.save(wxmenu);
	}
	/**
	 * 保存，新建或更新，有ID就更新，否则新建
	 ...
	 * @param wxmenu
	 */
	public void saveOrUpdate(User sessionUser,Wxmenu wxmenu){
		save(sessionUser,wxmenu);
	}
	/**
	 ...
	 * @param entities
	 */
	public void saveOrUpdateAll(User sessionUser,Collection<Wxmenu> entities) {
		if(entities==null)
			return;
		for (Wxmenu wxmenu : entities) {
			save(sessionUser,wxmenu);
		}
	}
	/**
	 ...
	 * @throws Exception
	 */
	public void deleteAll(User sessionUser,List<Wxmenu> list) throws BoException,Exception{
		for (Wxmenu obj : list) {
	 ...
	 */
	public void csindex(User sessionUser,Wxmenu obj,String act) throws BoException,Exception{
	 ...
	 * @return
	 */
	public Grid csgrid(User sessionUser,Wxmenu search, PageHelper ph,String queryhq) throws BoException,Exception{
	 ...
	 * @throws Exception
	 */
	public void saveCreate(User sessionUser,Wxmenu oldobj,Wxmenu obj,String copyfrom,String copyfromwhere) throws BoException,Exception{
		//用于做复制新增的检查
		...
		//创建_写表前操作之通用
		commonzSvrUitlsSpec.commonCreateBefore(oldobj,obj,"Wxmenu",sessionUser);
		//创建_写表前操作之本地
	 ...
	 * @throws Exception
	 */
	public void updateUpdate(User sessionUser,Wxmenu obj,Integer id) throws BoException,Exception{
		...
		commonzSvrUitlsSpec.commonUpdateBefore(dbobj,obj,"Wxmenu",sessionUser);
	 ...
	 * @throws Exception
	 */
	public void saveCreateUpdateUpdate(User sessionUser,Wxmenu oldobj,Wxmenu obj) throws BoException,Exception{
		if(null==obj.getId()){
	 ...
	 * @return
	 */
	public Wxmenu csshow(User sessionUser,Integer id) throws BoException,Exception{
	 ...
	 * @return
	 */
	public Wxmenu csnewget(User sessionUser,Wxmenu wxmenu,String copyfrom,String copyfromwhere) throws BoException,Exception{
		if(wxmenu==null)
			wxmenu=new Wxmenu();
		//如果是从其它表复制过来的
		if(StringUtils.isNotBlank(copyfrom)&&StringUtils.isNotBlank(copyfromwhere)){
	 ...
	 * @throws Exception
	 */
	public Wxmenu getOrNew(User sessionUser,Integer id) throws BoException,Exception{
		Wxmenu obj=get(sessionUser, id);
		if(obj==null){
			obj = new Wxmenu(
				null , //Integer 卖家   
				null , //String 菜单类型   {'menu':'菜单','button':'一级菜单','sub_button':'二级菜单'}
				null , //String 响应动作类型  菜单的响应动作类型 {'click':'点击推时间','view':'跳转URL','scancode_push':'扫码推事件','scancode_waitmsg':'扫码推事件且弹出消息接收中提示框','pic_sysphoto':'弹出系统拍照发图','pic_photo_or_album':'弹出拍照或者相册发图','pic_weixin':'弹出微信相册发图器','location_select':'弹出地理位置选择器','media_id':'下发消息（除文本消息）','view_limited':'跳转图文消息URL'}
				null , //String 菜单标题  菜单标题，不超过16个字节，子菜单不超过40个字节 
				null , //String 菜单KEY值  click等点击类型必须 菜单KEY值，用于消息接口推送，不超过128字节 
				null , //String 网页链接  view类型必须 网页链接，用户点击菜单可打开链接，不超过256字节 
				null , //String 合法media_id  调用新增永久素材接口返回的合法media_id 
	 ...
	 * @throws Exception
	 */
	public Wxmenu csedit(User sessionUser,Integer id) throws BoException,Exception{
	 ...
	 * @throws Exception
	 */
	public List<Tree> cstree(User sessionUser,Wxmenu search, PageHelper ph,String queryhq) throws BoException,Exception{
		...
		List<Wxmenu> listWxmenu=listfind(sessionUser, where, null, params.toArray());
	 ...
	 * @throws Exception
	 */
	public Object cstreegrid(User sessionUser,Wxmenu search, PageHelper ph,String queryhq,Integer id) throws BoException,Exception{
		if(id==null)
			id=0;
	...
	public void addList(User sessionUser,Wxmenu obj) throws BoException,Exception{
		if(obj==null)return;
		 //这里不要查session
		//这个以后要去掉的
		obj.setMtypeMap((Map)MapDb.getInstance().getMap("Wxmenu.Mtype"));
		//这个以后要去掉的
		obj.setTypeMap((Map)MapDb.getInstance().getMap("Wxmenu.Type"));
	 ...
	 * @throws Exception
	 */
	public void addMore(User sessionUser,Wxmenu obj) throws BoException,Exception{
		if(obj==null)return;
	...
	public void addString(User sessionUser,Wxmenu obj) throws BoException,Exception{
		...
		value=(String)MapDb.getInstance().getMapString("Wxmenu.Mtype", ""+obj.getMtype());
		if(StringUtils.isEmpty(value))
			value=""+obj.getMtype();
		obj.setMtypeString(value);
		value=(String)MapDb.getInstance().getMapString("Wxmenu.Type", ""+obj.getType());
		if(StringUtils.isEmpty(value))
			value=""+obj.getType();
	...
	public List<Wxmenu> listWithTree(boolean addprent) throws BoException,Exception{
		...
		List<Wxmenu> listAll=listfind(null, queryHelper.getWhere(), queryHelper.getOrder(), null);
