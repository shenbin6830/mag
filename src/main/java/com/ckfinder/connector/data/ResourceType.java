...
public class ResourceType {
	 ...
	 * @return the url
	 */
	public final String getUrl(HttpServletRequest request) throws ConnectorException{
		String up=ZresPathUtil.getBaseUrlAndPath(request);
		if(up==null)
			throw new ConnectorException(
					Constants.Errors.CKFINDER_CONNECTOR_ERROR_UNAUTHORIZED,
					false);
		String ret=this.url.replaceAll(Constants.BASE_URL_PLACEHOLDER, Constants.BASE_URL+up);
	 ...
	 */
	public final String getPath(HttpServletRequest request) throws ConnectorException {
		String up=ZresPathUtil.getBaseUrlAndPath(request);
		if(up==null)
			throw new ConnectorException(
					Constants.Errors.CKFINDER_CONNECTOR_ERROR_UNAUTHORIZED,
					false);
		String ret=this.path.replaceAll(Constants.BASE_DIR_PLACEHOLDER, Constants.BASE_DIR+up);
		ret=PathUtils.escape(ret);
		ret=PathUtils.addSlashToEnd(ret);
		return ret;
	}
