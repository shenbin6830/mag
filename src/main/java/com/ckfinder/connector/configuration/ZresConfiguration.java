 ...
 */
public class ZresConfiguration {

	/** 日志实例 */
	private final Log logger = LogFactory.getLog(getClass());

	private static ZresConfiguration instance;

	public static ZresConfiguration getInstace() {
		if (instance == null) {
			instance = new ZresConfiguration();
		}
		return instance;
	}
	...
	protected ArrayList<String> typesOrder = new ArrayList<String>();
	...
	public static int DEFAULT_IMG_HEIGHT = 400;
	public static int DEFAULT_THUMB_MAX_WIDTH = 100;
	public static int DEFAULT_THUMB_MAX_HEIGHT = 100;
	public static float DEFAULT_IMG_QUALITY = 0.8f;
	public static String DEFAULT_THUMBS_URL = "_thumbs/";
	public static String DEFAULT_THUMBS_DIR = "%BASE_DIR%_thumbs/";
	public static boolean DEFAULT_CHECKAUTHENTICATION = true;
	public static String DEFAULT_URI_ENCODING = "UTF-8";
	public static String DEFAULT_BASE_URL = "/userfiles";

	/** xml的位置，启动时赋一下值 */
	public static String xmlpath = "/opt/prj/zres.xml";
	 ...
	 */
	public ZresConfiguration() {

		this.xmlFilePath = xmlpath;// PathUtils.escape(SpringInit.getServletContext().getRealPath(""))+"/WEB-INF/classes/zres.xml";
	...
	public void init() throws Exception {
		...
		this.baseDir = Constants.BASE_DIR;
		this.baseURL = Constants.BASE_URL;

		String cfgpath = getFullConfigPath();
		...
		if (node != null) {
			...
			for (int i = 0; i < nodeList.getLength(); i++) {
				...
				if (childNode.getNodeName().equals("enabled")) {
					this.enabled = Boolean.valueOf(childNode.getTextContent()
							.trim());
				}
				...
				if (childNode.getNodeName().equals("checkSizeAfterScaling")) {
					this.checkSizeAfterScaling = Boolean.valueOf(childNode
							.getTextContent().trim());
				}
				...
				if (childNode.getNodeName().equals("secureImageUploads")) {
					this.secureImageUploads = Boolean.valueOf(childNode
							.getTextContent().trim());
				}
	...
	protected void registerEventHandlers() {
		for (PluginInfo item : this.plugins) {
			try {
				...
				Class<Plugin> clazz = (Class<Plugin>) Class.forName(item
						.getClassName());
	...
	private AccessControlLevel getACLFromNode(final Node childNode) {
		...
		for (int i = 0; i < childNode.getChildNodes().getLength(); i++) {
			...
			if (childChildNode.getNodeName().equals("folderView")) {
				acl.setFolderView(Boolean.valueOf(childChildNode
						.getTextContent().trim()));
			}
			if (childChildNode.getNodeName().equals("folderCreate")) {
				acl.setFolderCreate(Boolean.valueOf(childChildNode
						.getTextContent().trim()));
			}
			if (childChildNode.getNodeName().equals("folderRename")) {
				acl.setFolderRename(Boolean.valueOf(childChildNode
						.getTextContent().trim()));
			}
			if (childChildNode.getNodeName().equals("folderDelete")) {
				acl.setFolderDelete(Boolean.valueOf(childChildNode
						.getTextContent().trim()));
			}
			if (childChildNode.getNodeName().equals("fileView")) {
				acl.setFileView(Boolean.valueOf(childChildNode.getTextContent()
						.trim()));
			}
			if (childChildNode.getNodeName().equals("fileUpload")) {
				acl.setFileUpload(Boolean.valueOf(childChildNode
						.getTextContent().trim()));
			}
			if (childChildNode.getNodeName().equals("fileRename")) {
				acl.setFileRename(Boolean.valueOf(childChildNode
						.getTextContent().trim()));
			}
			if (childChildNode.getNodeName().equals("fileDelete")) {
				acl.setFileDelete(Boolean.valueOf(childChildNode
						.getTextContent().trim()));
			}
		}
	...
	private void setThumbs(final NodeList childNodes) {
		for (int i = 0; i < childNodes.getLength(); i++) {
			Node childNode = childNodes.item(i);
			if (childNode.getNodeName().equals("enabled")) {
				this.thumbsEnabled = Boolean.valueOf(childNode.getTextContent()
						.trim());
			}
			...
			if (childNode.getNodeName().equals("directAccess")) {
				this.thumbsDirectAccess = Boolean.valueOf(childNode
						.getTextContent().trim());
			}
	...
	private void setTypes(final Document doc) {
		...
		for (int i = 0; i < list.getLength(); i++) {
			...
			if (name != null && !name.equals("")) {
				ResourceType resourceType = createTypeFromXml(name,
	...
	private ResourceType createTypeFromXml(final String typeName,
		...
		for (int i = 0; i < childNodes.getLength(); i++) {
			...
			if (childNode.getNodeName().equals("allowedExtensions")) {
				resourceType.setAllowedExtensions(childNode.getTextContent()
						.trim());
			}
			if (childNode.getNodeName().equals("deniedExtensions")) {
				resourceType.setDeniedExtensions(childNode.getTextContent()
						.trim());
			}
		}
		return resourceType;
	...
	public boolean checkAuthentication(final HttpServletRequest request) {
		 ...
		 * session.getServletContext(); ServletContext context1=
		 * context.getContext("/zuc"); System.out.println("context1="+context1);
		 * if(context1==null) return false; HttpSession session2
		 * =(HttpSession)context1.getAttribute("session"); if(session2==null)
		 * return false;
		 * //System.out.println("base传过来的user为:"+session2.getAttribute("name"));
		 * 
		 * 
		 * //zmax修改 //如果有管理员 String
		 * adminid=(String)session2.getAttribute(Zres.SESSIONADMIN);
		 * System.out.println("session.adminid=" + adminid); if(adminid!=null){
		 * session.setAttribute("CKFinder_UserRole", "admin"); return true;
		 * }else{ session.removeAttribute("CKFinder_UserRole"); } //如果有用户
		 * Integer siteid=(Integer)session2.getAttribute(Zres.SESSIONUSERID);
		 * System.out.println("session.siteid=" + siteid); if(siteid!=null){
		 * session.setAttribute("CKFinder_UserRole", "user"); return true;
		 * }else{ session.removeAttribute("CKFinder_UserRole"); } return false;
		 */
	 ...
	 * 
	 */
	public String getThumbsURL(final HttpServletRequest request)
			throws ConnectorException {
		String up = ZresPathUtil.getBaseUrlAndPath(request);
		if (up == null)
			throw new ConnectorException(
					Constants.Errors.CKFINDER_CONNECTOR_ERROR_UNAUTHORIZED,
					false);
		String ret = this.thumbsURL.replaceAll(Constants.BASE_URL_PLACEHOLDER,
				Constants.BASE_URL + up);
		ret = PathUtils.escape(ret);
		ret = PathUtils.addSlashToEnd(ret);
		return ret;
	}
	 ...
	 * 
	 */
	public String getThumbsDir(final HttpServletRequest request)
			throws ConnectorException {
		String up = ZresPathUtil.getBaseUrlAndPath(request);
		if (up == null)
			throw new ConnectorException(
					Constants.Errors.CKFINDER_CONNECTOR_ERROR_UNAUTHORIZED,
					false);
		String ret = this.thumbsDir.replaceAll(Constants.BASE_DIR_PLACEHOLDER,
				Constants.BASE_DIR + up);
		ret = PathUtils.escape(ret);
		ret = PathUtils.addSlashToEnd(ret);
		return ret;
	}
