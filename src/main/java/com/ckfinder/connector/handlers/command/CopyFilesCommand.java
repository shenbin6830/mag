...
public class CopyFilesCommand extends XMLCommand implements IPostCommand{
	 ...
	 */
	private int copyFiles(final HttpServletRequest request) throws IOException,ConnectorException {
		this.filesCopied = 0;
		...
		for (FilePostParam file : files) {
			...
			File sourceFile = new File(configuration.getTypes()
										.get(file.getType()).getPath(request)
										+ file.getFolder(), file.getName());
			File destFile = new File(configuration.getTypes()
										.get(this.type).getPath(request)
	...
	private boolean handleAutoRename(final File sourceFile, final File destFile)
		...
		while (true) {
			String filePath = FileUtils.getFileNameWithoutExtension(
	 ...
	 * @throws IOException when ioerror occurs
	 */
	private void copyThumb(final HttpServletRequest request,final FilePostParam file) throws IOException,ConnectorException {
		File sourceThumbFile = new File(configuration.getThumbsDir(request) + File.separator
												+ file.getType()
												+ file.getFolder(), file.getName());
		File destThumbFile = new File(configuration.getThumbsDir(request) + File.separator
												+ type
												+ this.currentFolder, file.getName());
	...
	public void initParams(final HttpServletRequest request,
		...
		this.copiedAll = (request.getParameter("copied") != null) ? Integer
				.valueOf(request.getParameter("copied")) : 0;
