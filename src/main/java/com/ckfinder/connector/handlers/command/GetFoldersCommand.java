...
public class GetFoldersCommand extends XMLCommand {
	...
	protected void createXMLChildNodes(final HttpServletRequest request,final int errorNum, final Element rootElement)
		...
		if (errorNum == Constants.Errors.CKFINDER_CONNECTOR_ERROR_NONE) {
			createFoldersData(request,rootElement);
		}
	}
	...
	private void filterListByHiddenAndNotAllowed() {
		...
		for (String dir : this.directories) {
			if (AccessControlUtil.getInstance(this.configuration)
									.checkFolderACL(this.type, this.currentFolder + dir,
													this.userRole,
									AccessControlUtil.CKFINDER_CONNECTOR_ACL_FOLDER_VIEW)
	 ...
	 */
	private void createFoldersData(final HttpServletRequest request,final Element rootElement) throws ConnectorException{
		Element element = creator.getDocument().createElement("Folders");
		for (String dirPath : directories) {
			File dir = new File(this.configuration.getTypes().get(this.type).getPath(request)
					+ this.currentFolder
					+ dirPath);
			if (dir.exists()) {
				XmlElementData xmlElementData = new XmlElementData("Folder");
				xmlElementData.getAttributes()
									.add(new XmlAttribute("name", dirPath));
				
                                xmlElementData.getAttributes()
									.add(new XmlAttribute("hasChildren",
						FileUtils.hasChildren(this.currentFolder+dirPath+"/", dir, configuration, this.type, this.userRole).toString()));


				xmlElementData.getAttributes()
								.add(new XmlAttribute("acl",
										 String.valueOf(AccessControlUtil
												 	.getInstance(configuration)
												 	.checkACLForRole(this.type,
												 					 this.currentFolder
												 					 + dirPath,
												 					 this.userRole))));
