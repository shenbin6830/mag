...
public class FileUploadCommand extends Command implements IPostCommand {
	...
	public void execute(final HttpServletRequest request,final OutputStream out) throws ConnectorException {
		...
		try {
			...
			if (!uploaded) {
				...
				path = configuration.getTypes().get(type).getUrl(request)
								+ this.currentFolder;
			}
			...
			if (this.responseType != null && this.responseType.equals("txt")) {
				...
				if (checkFuncNum()) {
					handleOnUploadCompleteCallFuncResponse(request,out, errorMsg, path);
				} else {
					handleOnUploadCompleteResponse(request,out, errorMsg);
				}
	 ...
	 */
	protected void handleOnUploadCompleteCallFuncResponse(final HttpServletRequest request,final OutputStream out,
	 ...
	 *             when error occurs
	 */
	protected void handleOnUploadCompleteResponse(final HttpServletRequest request,final OutputStream out,
			final String errorMsg) throws IOException , ConnectorException{
	...
	private boolean fileUpload1(final HttpServletRequest request) {
	...
	private boolean validateUploadItem(final FileItem item, final String path) {
		...
		//如果说要进行文件名转换，放在下面
		//有中文，换成时间文件名
		if(FileUtils.isContainsChinese(this.fileName)){
			this.newFileName=new SimpleDateFormat("yyyyMMddhhmmssSSS").format(new Date())+"."+FileUtils.getFileExtension(this.fileName);
		}
	...
	protected boolean checkIfCurrFolderExists(final HttpServletRequest request)
		...
		File currDir = new File(configuration.getTypes().get(tmpType).getPath(request)
				+ this.currentFolder);
