...
public abstract class XMLCommand extends Command {
	 ...
	 */
	public void execute(final HttpServletRequest request,final OutputStream out) throws ConnectorException {
		try {
			createXMLResponse(request,getDataForXml(request));
			out.write(creator.getDocumentAsText().getBytes("UTF-8"));
		} catch (ConnectorException e) {
			throw e;
		} catch (IOException e) {
			throw new ConnectorException(Constants.Errors.
					CKFINDER_CONNECTOR_ERROR_ACCESS_DENIED, e);
		}
	}
	 ...
	 * @throws ConnectorException to handle in error handler.
	 */
	private void createXMLResponse(final HttpServletRequest request,final int errorNum) throws ConnectorException {
		...
		if (mustAddCurrentFolderNode()) {
			createCurrentFolderNode(request,rootElement);
		}
		creator.addErrorCommandToRoot(rootElement, errorNum, getErrorMsg(errorNum));
		createXMLChildNodes(request,errorNum, rootElement);
	 ...
	 * @throws ConnectorException to handle in error handler.
	 */
	protected abstract void createXMLChildNodes(final HttpServletRequest request,final int errorNum,
												final Element rootElement)
														throws ConnectorException;
	/**
	 * gets all necessary data to create XML response.
	 * @return error code {@link com.ckfinder.connector.configuration.Constants.Errors} or {@link com.ckfinder.connector.configuration.Constants.Errors#CKFINDER_CONNECTOR_ERROR_NONE} if no error occurred.
	 */
	protected abstract int getDataForXml(final HttpServletRequest request) throws ConnectorException;
	 ...
	 *           XML root node.
	 */
	protected void createCurrentFolderNode(final HttpServletRequest request,final Element rootElement) throws ConnectorException {
		Element element = creator.getDocument().createElement("CurrentFolder");
		element.setAttribute("path", this.currentFolder);
		element.setAttribute("url", configuration.getTypes().get(this.type).getUrl(request)
									+ this.currentFolder);
		element.setAttribute("acl", String.valueOf(AccessControlUtil
				.getInstance(configuration).checkACLForRole(this.type,
	...
	public void initParams(final HttpServletRequest request,
			final ZresConfiguration configuration, final Object... params)
			throws ConnectorException {
