...
public class GetFilesCommand extends XMLCommand {
	...
	protected void createXMLChildNodes(final HttpServletRequest request,final int errorNum, final Element rootElement)
		...
		if (errorNum == Constants.Errors.CKFINDER_CONNECTOR_ERROR_NONE) {
			createFilesData(request,rootElement);
		}
	}
	...
	protected int getDataForXml(final HttpServletRequest request) throws ConnectorException{
		...
		this.fullCurrentPath = configuration.getTypes().get(this.type).getPath(request)
										+ this.currentFolder;
	...
	private void filterListByHiddenAndNotAllowed() {
		...
		for (String file : this.files) {
			if (FileUtils.checkFileExtension(file, this.configuration
					.getTypes().get(this.type), this.configuration,
					false) == 0
	 ...
	 */
	private void createFilesData(final HttpServletRequest request,final Element rootElement) throws ConnectorException{
		...
		for (String filePath : files) {
			...
			if (file.exists()) {
				...
				if (ImageUtils.isImage(file) && isAddThumbsAttr()) {
					String attr = createThumbAttr(request,file);
					if (!attr.equals("")) {
	 ...
	 *
	 * @param file
	 *            file to check if has thumb.
	 * @return thumb attribute values
	 */
	private String createThumbAttr(final HttpServletRequest request,final File file) throws ConnectorException{
		File thumbFile = new File(configuration.getThumbsDir(request)
	...
	private boolean isShowThumbs() {
		return (this.showThumbs != null && this.showThumbs
				.toString().equals("1"));
	}
