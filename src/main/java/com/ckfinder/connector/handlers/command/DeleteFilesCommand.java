...
public class DeleteFilesCommand extends XMLCommand implements IPostCommand{
	...
	@Override
	protected void createXMLChildNodes(final HttpServletRequest request,int errorNum, Element rootElement ) throws ConnectorException {
		...
		for (FilePostParam fileItem : this.files) {
			...
			if (FileUtils.checkIfFileIsHidden(fileItem.getName(), this.configuration)) {
				return Constants.Errors.CKFINDER_CONNECTOR_ERROR_INVALID_REQUEST;
                        ...
                        if (FileUtils.checkFileExtension(fileItem.getName(), this.configuration.getTypes().get(fileItem.getType()),
			...
			if (!AccessControlUtil.getInstance(this.configuration)
						.checkFolderACL(fileItem.getType(), fileItem.getFolder(), this.userRole,
							AccessControlUtil.CKFINDER_CONNECTOR_ACL_FILE_DELETE)) {
				return Constants.Errors.CKFINDER_CONNECTOR_ERROR_UNAUTHORIZED;
			}
                        
                        File file = new File(configuration.getTypes().get(fileItem.getType()).getPath(request)+fileItem.getFolder(), fileItem.getName());
	...
	public void initParams( HttpServletRequest request,
					ZresConfiguration configuration,
