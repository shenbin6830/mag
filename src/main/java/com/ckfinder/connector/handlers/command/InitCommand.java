...
import com.ckfinder.connector.configuration.ZresPathUtil;
...
public class InitCommand extends XMLCommand {
	 ...
	 */
	@Override
	protected int getDataForXml(final HttpServletRequest request) {
		try {
			ZresPathUtil.init(request);
		} catch (Exception e) {
			// e.printStackTrace();
			//logger.error(e);
		}
	...
	@Override
	protected void createXMLChildNodes(final HttpServletRequest request,final int errorNum,
			final Element rootElement)
	throws ConnectorException {
		if (errorNum == Constants.Errors.CKFINDER_CONNECTOR_ERROR_NONE) {
			createConnectorData(request,rootElement);
			try {
				createResouceTypesData(request,rootElement);
			} catch (Exception e) {
	 ...
	 */
	private void createConnectorData(final HttpServletRequest request,final Element rootElement) throws ConnectorException {
		// connector info
		...
		{
			element.setAttribute("thumbsUrl", configuration.getThumbsURL(request));
	 ...
	 */
	private void createResouceTypesData(final HttpServletRequest request,final Element rootElement) throws Exception {
		//resurcetypes
		...
		for (String key : types) {
			...
			if (((this.type == null || this.type.equals(key)) && resourceType != null)
					&& AccessControlUtil.getInstance(this.configuration)
					.checkFolderACL(key, "/", this.userRole,
							AccessControlUtil.CKFINDER_CONNECTOR_ACL_FOLDER_VIEW)) {


			 * */
			//非管理员不放users
			if(key.equals("users") && !"admin".equals(request.getSession().getAttribute(SessionName.ZRES_SESSIONUSERTYPE))){
				continue;
			}
			if (((this.type == null || this.type.equals(key)) && resourceType != null)) {

				Element childElement = creator.getDocument().createElement("ResourceType");
				childElement.setAttribute("name", resourceType.getName());
				//childElement.setAttribute("acl", "255");
				childElement.setAttribute("acl", String.valueOf(AccessControlUtil.getInstance(configuration).checkACLForRole(key, "/", this.userRole)));
				childElement.setAttribute("hash", randomHash(resourceType.getPath(request)));
				...
				childElement.setAttribute("url", resourceType.getUrl(request));
				...
				childElement.setAttribute("hasChildren",
						FileUtils.hasChildren("/", new File(PathUtils.escape(resourceType.getPath(request))),
