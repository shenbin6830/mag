...
public class CreateFolderCommand extends XMLCommand implements IPostCommand{
	 ...
	 */
	private boolean createFolder(final HttpServletRequest request) throws ConnectorException {
		File dir = new File(configuration.getTypes().get(type).getPath(request)
				+ currentFolder + newFolderName);
		if (dir.exists()) {
			throw new ConnectorException(Constants
									.Errors.CKFINDER_CONNECTOR_ERROR_ALREADY_EXIST);
		} else {
