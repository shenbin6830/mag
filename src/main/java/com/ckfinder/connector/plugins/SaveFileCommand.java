...
public class SaveFileCommand extends XMLCommand implements IEventHandler {
	...
	protected int getDataForXml(final HttpServletRequest request) throws ConnectorException{
		File sourceFile = new File(configuration.getTypes().get(this.type).getPath(request)
															+ this.currentFolder,
															this.fileName);
	...
	public boolean runEventHandler(EventArgs args, ZresConfiguration configuration1)
			throws ConnectorException {
	...
	public void initParams(HttpServletRequest request,
			ZresConfiguration configuration, Object... params)
			throws ConnectorException {
