...
public class ImageResizeCommad extends XMLCommand implements IEventHandler {
	...
	public boolean runEventHandler(EventArgs eventArgs, ZresConfiguration configuration1)
	...
	@Override
	protected void createXMLChildNodes(final HttpServletRequest request,int arg0, Element arg1)
			throws ConnectorException {
	...
	protected int getDataForXml(final HttpServletRequest request) throws ConnectorException{
		...
		File file = new File(configuration.getTypes().get(type).getPath(request) + this.currentFolder,
								fileName);
		try {
			...
			if (this.width != null && this.height != null) {
				...
				File thumbFile =  new File(configuration.getTypes().get(type).getPath(request) + this.currentFolder,
																this.newFileName);
			...
			for (String size : SIZES) {
				if (sizesFromReq.get(size) != null
					...
					String thumbName = fileNameWithoutExt
													...
													.concat(".")
													.concat(fileExt);
					File thumbFile = new File(configuration.getTypes().get(this.type).getPath(request)
	...
	public void initParams(HttpServletRequest request,
			ZresConfiguration configuration1, Object... params)
			throws ConnectorException {
