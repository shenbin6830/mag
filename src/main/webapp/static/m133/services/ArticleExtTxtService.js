 ...
 */
app
    .service('ArticleExtTxtService', function($rootScope,$q,$http,$log,ENV) {
        ...
        return {
			...
			/**
             * 创建Promise版
             * @param ArticleExtTxt 要被创建的对象
             * @param _page 页码对象，如果为空使用全局页码对象
             */
            create: function(ArticleExtTxt,_page) {
                var thispage=(_page)?_page:_Page;
                var tmpparam=_.omit(ArticleExtTxt, function(value, key, object) {
                    return _.isObject(value) || _.isArray(value)
                        || key.indexOf("String") != -1 || key.indexOf("gmt") != -1
                        ;
                });
                tmpparam=_.extend(tmpparam, {cmd: thispage.cmd});
                $log.debug("ArticleExtTxtService create");
                $log.debug(tmpparam);
                var deferred = $q.defer();
                var url =aurl+"/create";
                $http({
            ...
            /**
             * 更新Promise版
             * @param ArticleExtTxt 要被更新的对象
             * @param _page 页码对象，如果为空使用全局页码对象
             */
            update: function(ArticleExtTxt,_page) {
                var thispage=(_page)?_page:_Page;
                var tmpparam=_.omit(ArticleExtTxt, function(value, key, object) {
                    return _.isObject(value) || _.isArray(value)
                        || key.indexOf("String") != -1 || key.indexOf("gmt") != -1
